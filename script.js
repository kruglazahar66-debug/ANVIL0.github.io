...
// Получение embed-ссылки (безопасной)
...
// Получение прямой ссылки для открытия в новой вкладке
function getDirectUrl(url) {
    if (!url) return '';
    // Если уже embed, пытаемся восстановить оригинал
...
function initData() {
    ...
    if (!storedMatches) {
        ...
        const defaultMatches = [
            ...
            { id: 'm2', title: '🎮 ESL One Dota 2 • Финал', datetime: tomorrow.toISOString().slice(0, 16), streamUrl: 'https://www.twitch.tv/riotgames', preview: 'Team Spirit vs Gaimin' },
    ...
    upcoming.forEach(m => {
        html += `
            <div class="match-card" onclick="watchStream('${m.id}')">
                ...
                <div class="match-preview"><i class="fas fa-chart-simple"></i> ${escapeHtml(m.preview)}</div>
                <button class="stream-btn"><i class="fas fa-ticket"></i> Запланировано</button>
            </div>
        `;
    });
    html += `</div>`;
    ...
    appDiv.innerHTML = `
        ...
        <div class="player-container">
            ...
            <div style="background:#0a0a0f; padding:14px; font-size:14px; color:#aaa; text-align:center; border-top:1px solid #222;">
                ...
                <strong>Если видео не загружается</strong> — нажмите кнопку <strong>"Открыть в новой вкладке"</strong> выше. 
                Это обходит ограничения браузера.
            </div>
        </div>
        <script>
            // Автоматическое обнаружение ошибки загрузки iframe
            const iframe = document.getElementById('streamFrame');
            let errorTimeout = setTimeout(() => {
                const container = iframe?.parentElement;
                if (container && !iframe.src.includes('about:blank')) {
                    const warning = document.createElement('div');
                    warning.className = 'iframe-error-message';
                    warning.innerHTML = '<i class="fas fa-ban"></i> Не удалось загрузить плеер. <button onclick="openDirect(\'${escapeHtml(directUrl)}\')" class="player-action-btn" style="margin-left:10px;">Открыть видео</button>';
                    if (!container.querySelector('.iframe-error-message')) container.appendChild(warning);
                }
            }, 5000);
